    ...
    window.moModalChiTietKeo = async function (idCaDau) {
        ...
        try {
            ...
            ${(() => {
                ...
                if (!window.currentGuest) return `<div style="text-align:center;padding:4px 0;"><p style="font-size:0.82rem;color:#64748b;margin-bottom:10px;">Đăng nhập để đặt slot tham gia ca này.</p><button class="btn-dat-slot" style="width:100%;" onclick="window.dongModalChiTietKeo();setTimeout(()=>{if(window.innerWidth<768)window.openLoginSheet?.();else{var el=document.getElementById('guestAuthPanel');if(el)el.scrollIntoView({behavior:'smooth'});}},200)"><i class="fa-solid fa-right-to-bracket"></i> Đăng nhập / Đăng ký</button></div>`;
